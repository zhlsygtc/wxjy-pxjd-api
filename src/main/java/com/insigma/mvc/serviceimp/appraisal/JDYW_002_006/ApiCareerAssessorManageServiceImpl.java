package com.insigma.mvc.serviceimp.appraisal.JDYW_002_006;
...
import com.insigma.mvc.dao.appraisal.JDYW_002_006.ApiCareerAssessorManageMapper;
...
import com.insigma.mvc.service.appraisal.JDYW_002_006.ApiCareerAssessorManageService;
import com.insigma.mvc.service.common.fileupload.ApiFileUploadService;

@Service
@Transactional(rollbackFor=Exception.class)
public class ApiCareerAssessorManageServiceImpl extends MvcHelper implements ApiCareerAssessorManageService{

	@Resource
	private ApiCareerAssessorManageMapper apiCareerAssessorManageMapper;
	...
	//获取相应机构的考评员
	@Override
	public PageInfo<Hc73> getCareerAssessorList(Hc73 hc73) {
		...
		// 是考评员
		hc73.setCac022(HC73CAC022Enum.YESANDNO_YES.getCode());
		List<Hc73> list=apiCareerAssessorManageMapper.getCareerAssessorList(hc73);
	...
	//根据考评员内码获取考评员信息
	@Override
	public AjaxReturnMsg getCareerAssessorById(String id, String aab001) {
		Hc73 hc73 = apiCareerAssessorManageMapper.getCareerAssessorById(id, aab001, COMAAE100Enum.DATA_EFFECTIVE.getCode(), HC73CAC022Enum.YESANDNO_YES.getCode());
	...
	public AjaxReturnMsg<String> checkAac002(Hc73 hc73) {
		List<Hc73> list=apiCareerAssessorManageMapper.checkAac002(hc73.getAac002(), COMAAE100Enum.DATA_EFFECTIVE.getCode());
		if(list.size()>0){
			return this.error("此身份证已在考评员库");
		}else{
	...
	public AjaxReturnMsg saveData(Hc73 hc73) {
		...
		if(StringUtils.isBlank(hc73.getChc075())) {
			...
			if(list != null && list.size()>0){
				...
				checkHc73.setCac022(HC73CAC022Enum.YESANDNO_YES.getCode());
				checkHc73.setAac002(hc73.getAac002());
				Integer n = apiCareerAssessorManageMapper.getCheckAppointmentPresence(checkHc73);
				if (n != null && n > 0) {
					return this.error("该人员已存在与鉴定所站聘任关系，请勿重复新增");
				}
				// 存在该人员基本信息 但未获得工作人员身份类型 “考评员” 添加其考评员身份
//				checkHc73 = new Hc73();
//				checkHc73.setAac002(hc73.getAac002());
//				checkHc73.setAae100(COMAAE100Enum.DATA_EFFECTIVE.getCode());
//				checkHc73.setCac022(HC73CAC022Enum.YESANDNO_YES.getCode());
//				checkHc73.setAab001(hc73.getAab001());
				// 赋予用户考评员身份
//				int num = apiCareerAssessorManageMapper.updatePerponIdentity(checkHc73);
//				if (num != 1) {
//					return this.error("信息保存失败，请重试");
//				}
//				return this.success("新增考评员信息成功，请继续填写资质信息",hc73.getChc073());
			} else {
				// 该人员基本信息不存在 新增基本信息
				...
				// 是考评员
//				hc73.setCac022(HC73CAC022Enum.YESANDNO_YES.getCode());
				int num = apiCareerAssessorManageMapper.addCareerAssessor(hc73);
				if(num != 1) {
			...
			hc75.setCac022(HC73CAC022Enum.YESANDNO_YES.getCode());// 任聘关系 考评员
			// 新增聘任关系
			int num2 = apiCareerAssessorManageMapper.insertAppointment(hc75);
			if(num2 != 1) {
			...
			// 修改基本信息
			int num = apiCareerAssessorManageMapper.updateCareerAssessor(hc73);
			if(num != 1) {
			...
			hc75.setAae100(COMAAE100Enum.DATA_EFFECTIVE.getCode()); // 信息有效代码
			hc75.setAae011(hc73.getUserid()); //经办人
			hc75.setCac022(HC73CAC022Enum.YESANDNO_YES.getCode()); //考评员
			//只要是修改就需要审核
			hc75.setEae052(HC75EAE052Enum.WAIT_AUDIT.getCode()); //审核标志代码(通过后台新增的考评人员 默认通过审核)
			int num2 = apiCareerAssessorManageMapper.updateAppointment(hc75);
			if(num2 != 1) {
				return this.error("保存聘任关系信息失败");
			}
			return this.success("保存考评员信息成功",hc73.getChc073());
		}
	}

	//删除考评员信息(因为考评员基本信息共用关系 删除操作不删除基本信息 只解除雇佣关系)
	@Override
	public AjaxReturnMsg<String> deleteCareerAssessor(Hc73 hc73) {
		hc73 = apiCareerAssessorManageMapper.getCareerAssessorById(hc73.getChc073(), hc73.getAab001(), COMAAE100Enum.DATA_EFFECTIVE.getCode(), HC73CAC022Enum.YESANDNO_YES.getCode());
//		if(hc73.getBus_uuid()!=null) {
//			deleteFile(hc73.getBus_uuid());
//		}
		// 解除聘任关系
		int deletenum = apiCareerAssessorManageMapper.deleteEmployment(hc73.getChc073(), hc73.getAab001(), COMAAE100Enum.DATA_INVALID.getCode(), HC73CAC022Enum.YESANDNO_YES.getCode());
		if(deletenum == 1) {
			//资质删除
//			String[] ids = {hc73.getChc073()};
//			deletenum = apiCareerAssessorManageMapper.deleteQualifications(ids);
...
//		}
		...
		int batdeletenum=apiCareerAssessorManageMapper.batDeleteEmployment(hc73Map);
		if(batdeletenum==hc73.getA_chb061().length){
//			apiCareerAssessorManageMapper.deleteQualifications(hc73.getA_chb061());
	...
	public PageInfo<Hc74> getQualificationList(Hc74 hc74) {
		...
		List<Hc74> list=apiCareerAssessorManageMapper.getQualificationList(hc74.getChb061());
	...
	public AjaxReturnMsg saveQualification(Hc74 hc74) {
		...
		List<Hc74> list = apiCareerAssessorManageMapper.checkQualification(hc74);
		if(list.size()>0) {
			return this.error("此考评员已拥有该资质，无法重复保存");
		}
		...
		if("".equals(hc74.getChc074())) {
			...
			int num = apiCareerAssessorManageMapper.saveQualification(hc74);
			if(num == 1) {
			...
			int num = apiCareerAssessorManageMapper.updateQualification(hc74);
			if(num == 1) {
				return this.success("保存成功", hc74.getChc074());
			}else{
				return this.error("保存失败");
	...
	public AjaxReturnMsg<String> deleteQualification(String chc074) {
		Hc74 hc74 = apiCareerAssessorManageMapper.getQualification(chc074);
		if(hc74.getBus_uuid()!=null) {
			deleteFile(hc74.getBus_uuid());
		}
		int deletenum = apiCareerAssessorManageMapper.deleteQualification(chc074);
		if(deletenum == 1) {
			return this.success("删除成功");
		}else{
			return this.error("删除失败");
	...
	//根据考评员内码获取考评员信息中文
	@Override
	public AjaxReturnMsg getCareerAssessorGBKById(String chb061, String aab001) {
		Hc73 hc73 = apiCareerAssessorManageMapper.getCareerAssessorGBKById(chb061, aab001, COMAAE100Enum.DATA_EFFECTIVE.getCode(), HC73CAC022Enum.YESANDNO_YES.getCode());
		if(StringUtil.isNotEmpty(hc73.getPhoto_url())){
			//将图片域名拼接上去
			String fileModule = AppConfig.getProperties("fileModule");
			hc73.setPhoto_url(fileModule+hc73.getPhoto_url());
	...
	public AjaxReturnMsg getIdcardInfo(String aac002) {
		Hc73 hc73 = apiCareerAssessorManageMapper.getIdcardInfo(aac002, COMAAE100Enum.DATA_EFFECTIVE.getCode());
		if(StringUtil.isNotEmpty(hc73.getPhoto_url())){
			//将图片域名拼接上去
			String fileModule = AppConfig.getProperties("fileModule");
			hc73.setPhoto_url(fileModule+hc73.getPhoto_url());
	...
	//查询考评员教学信息
	@Override
	public PageInfo<Hb68> getCareerAssessorClasses(Hc73 hc73) {
		PageHelper.offsetPage(hc73.getOffset(), hc73.getLimit());
		List<Hb68> list=apiCareerAssessorManageMapper.getCareerAssessorClasses(hc73);
